...
void Renderer::update_dimensions()
{
	...
	{
		map_->get_stations()[i]->set_position(station_positions[i][0] * window_width_, station_positions[i][1] * window_height_);
	}
...
void Renderer::draw_stations()
{
	for(unsigned int i=0; i < map_->get_stations().size(); i++)
	{
		draw_station(map_->get_stations()[i]);
	}
...
{
	draw_rectangle(station->get_x() - STATION_WIDTH, station->get_y() - STATION_WIDTH/2, station->get_x() + STATION_WIDTH, station->get_y() + STATION_WIDTH/2);
...
void Renderer::draw_tracks()
{
	...
	{
		mvvline(track->to_.lock()->get_y() + STATION_WIDTH/2+1, track->from_.lock()->get_x() , ACS_VLINE, track->from_.lock()->get_y() - track->to_.lock()->get_y() - STATION_WIDTH - 1);
		mvhline(track->from_.lock()->get_y(), track->from_.lock()->get_x() + STATION_WIDTH + 1, ACS_HLINE, track->to_.lock()->get_x() - track->from_.lock()->get_x() - STATION_WIDTH*2);
	}
}
...
void Renderer::draw_train(const std::shared_ptr<Train> &train)
{
	if(train->on_station())
	{
//		mvaddch(train->get_station().lock()->get_y(), train->get_station().lock()->get_x(), train->id_+48);
	}
	else
	{
		mvaddch(train->get_station().lock()->get_y() +  (train->get_destination().lock()->get_y() - train->get_station().lock()->get_y()) * train->get_dist_pct(),
				train->get_station().lock()->get_x() +  (train->get_destination().lock()->get_x() - train->get_station().lock()->get_x()) * train->get_dist_pct(), train->id_+48);
	}
